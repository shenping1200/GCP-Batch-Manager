...
3. 选择区域：
   - **免费区**：可在爱荷华 南卡罗莱纳 俄勒冈 中手动选择
   - 也可以选择随机模式，此模式下只在以上三个区域随机选择创建实例
   - **付费区**：选择特定付费区域，也可选择随机创建，根据自己需求选择即可
4. 设置实例数量**：每个区域最多创建4台实例，按需选择，一个基础GCP项目默认最多开启12个实例，自由配置即可
5. 点击"开始部署"：等待机器开通即可
6. 此开机程序默认开通的是Ubuntu 22.04版本，其他系统未作设置，请熟悉
